...
## 구현한 기능
- **상태 관리**: 렌더링할 요소(유저의 선택, 컴퓨터의 선택, 게임 결과)를 상태로 관리하여 실시간으로 UI를 업데이트합니다.
- **컴포넌트화**: 반복되는 구조의 요소를 컴포넌트로 분리하여 재사용성을 높였습니다.
- **속성 전달**: 컴포넌트에 속성(`props`)을 전달하여 동적인 UI를 구성하고, 선택에 따라 화면을 즉시 렌더링합니다.
